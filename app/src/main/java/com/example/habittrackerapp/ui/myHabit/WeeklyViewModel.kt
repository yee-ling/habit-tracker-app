...
class WeeklyViewModel(repo: HabitsRepoRoom) : BaseFrequencyViewModel(repo) {
    init {
        getHabits()
    }
    override fun getHabits() {
        viewModelScope.launch(Dispatchers.IO) {
            repo.getAllHabits().map { habits ->
                habits.filter { it.frequency == Frequency.WEEKLY }
            }.collect { weeklyHabits ->
                _habits.value = weeklyHabits
            }
        }
    }
